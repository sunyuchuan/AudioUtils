...
#include "ijkj4a/MediaFormat.h"
#include "ijkj4a/ByteBuffer.util.h"
...
static void SDL_AMediaFormatJava_setInt32(SDL_AMediaFormat* aformat, const char* name, int32_t value)
{
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        LogError("%s: CallVoidMethod: failed", __func__);
        return;
    }
}

static void SDL_AMediaFormatJava_setString(SDL_AMediaFormat* aformat, const char* name, const char* value)
{
    LogInfo("%s", __func__);
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
    ...
    J4AC_android_media_MediaFormat__setString__withCString(env, android_media_format, name, value);
    if (J4A_ExceptionCheck__catchAll(env)) {
        LogError("%s: CallVoidMethod: failed", __func__);
        return;
    }
...
static void SDL_AMediaFormatJava_setBuffer(SDL_AMediaFormat* aformat, const char* name, void* data, size_t size)
{
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        LogError("%s: call jmid_setByteBuffer: failed", __func__);
        return;
    }
}
...
static void setup_aformat(SDL_AMediaFormat *aformat, jobject android_media_format) {
    ...
    aformat->func_setString = SDL_AMediaFormatJava_setString;
...
SDL_AMediaFormat *SDL_AMediaFormatJava_createFormat(JNIEnv *env)
{
    LogInfo("%s", __func__);

    jobject android_media_format = J4AC_MediaFormat__MediaFormat__asGlobalRef__catchAll(env);
    if (J4A_ExceptionCheck__catchAll(env) || !android_media_format) {
        return NULL;
    }
