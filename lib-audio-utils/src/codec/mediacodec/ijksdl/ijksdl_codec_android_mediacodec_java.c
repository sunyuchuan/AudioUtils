...
SDL_AMediaFormat *SDL_AMediaCodecJava_getOutputFormat(SDL_AMediaCodec *thiz)
{
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        LogError("%s: SetupThreadEnv failed", __func__);
        return NULL;
    }
...
static sdl_amedia_status_t SDL_AMediaCodecJava_delete(SDL_AMediaCodec* acodec)
{
    ...
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        LogError("SDL_AMediaCodecJava_delete: SetupThreadEnv failed");
    ...
    uint32_t flags)
{
    ...
    LogInfo("configure acodec:%p format:%p: surface:%p", android_media_codec, android_media_format, android_surface);
...
static sdl_amedia_status_t SDL_AMediaCodecJava_start(SDL_AMediaCodec* acodec)
{
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        LogError("%s: start failed", __func__);
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
...
static ssize_t SDL_AMediaCodecJava_readOutputData(SDL_AMediaCodec* acodec, size_t idx, uint8_t **data)
{
    ssize_t read_ret = -1;
...

    {
        ...
        read_ret = buf_size;
        //maybe use av_mallocz is better.
        *data = (uint8_t *)malloc(buf_size);
        memcpy((void *)*data, buf_ptr, read_ret);
    }
...
ssize_t SDL_AMediaCodecJava_dequeueInputBuffer(SDL_AMediaCodec* acodec, int64_t timeoutUs)
{
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        LogError("%s: dequeueInputBuffer failed", __func__);
...
sdl_amedia_status_t SDL_AMediaCodecJava_queueInputBuffer(SDL_AMediaCodec* acodec, size_t idx, off_t offset, size_t size, uint64_t time, uint32_t flags)
{
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        LogError("SDL_AMediaCodecJava_queueInputBuffer: SetupThreadEnv failed");
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
...
ssize_t SDL_AMediaCodecJava_dequeueOutputBuffer(SDL_AMediaCodec* acodec, SDL_AMediaCodecBufferInfo *info, int64_t timeoutUs)
{
    ...
    while (1) {
        ...
        if (idx == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
            LogInfo("%s: INFO_OUTPUT_BUFFERS_CHANGED\n", __func__);
            continue;
        } else if (idx == AMEDIACODEC__INFO_OUTPUT_FORMAT_CHANGED) {
            LogInfo("%s: INFO_OUTPUT_FORMAT_CHANGED\n", __func__);
        } else if (idx >= 0) {
            if (info) {
...
sdl_amedia_status_t SDL_AMediaCodecJava_releaseOutputBuffer(SDL_AMediaCodec* acodec, size_t idx, bool render)
{
    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        LogError("%s(%d, %s): SetupThreadEnv failed", __func__, (int)idx, render ? "true" : "false");
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
    ...
    if (J4A_ExceptionCheck__catchAll(env)) {
        LogError("%s: releaseOutputBuffer\n", __func__);
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
...
SDL_AMediaCodec* SDL_AMediaCodecJava_createEncoderByType(JNIEnv *env, const char *type)
{
    LogInfo("%s", __func__);

    jobject android_media_codec = J4AC_android_media_MediaCodec__createEncoderByType__withCString(env, type);
    if (J4A_ExceptionCheck__catchAll(env) || !android_media_codec) {
        return NULL;
    }
